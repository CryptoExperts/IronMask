...
// Splits the dependency number |idx| into 2 dependencies: one for the
// first input, the other for the second input. For instance:
//   ((a0 ^ r0) & (b0 ^ r1))
// Would be split into "a0 ^ r0" and "b0 ^ r1".
//
// Those new dependencies are put in the |deps1| and |deps2| arrays,
// at indices |*deps1_length| and |*deps2_length|.
//
// |is_first_level| is used to differentiate between toplevel calls to
// this function, where |idx| identifies a dependency in the tuple
// (and thus is in |local_deps|), and recursive calls to this
// function, where |idx| comes from a multiplication (and therefore is
// not in |local_deps|).
void update_mult_deps(const Circuit* c, BitDepVector** bit_deps,
                      BitDep** deps1, BitDep** deps2,
                      int* deps1_length, int* deps2_length,
                      BitDep* curr_dep,
                      const uint64_t* bit_i1_rands, const uint64_t* bit_i2_rands) {
  DependencyList* deps = c->deps;
  int rand_count       = c->random_count + c->secret_count;
  int mult_count       = deps->mult_deps->length;
  int bit_rand_len     = 1 + (rand_count / 64);
  int bit_mult_len     = 1 + (mult_count / 64);

  // Checking if |curr_dep| contains randoms used to refresh i1 or i2
  // or secret shares of i1 or i2. If so, then |deps1| or |deps2| are
  // updated with |curr_dep|.
  int has_i1_rand = curr_dep->secrets[0];
  int has_i2_rand = curr_dep->secrets[1];
  for (int i = 0; i < bit_rand_len && !has_i1_rand && !has_i2_rand; i++) {
    if (curr_dep->randoms[i] & bit_i1_rands[i]) {
      has_i1_rand = 1;
    } else if (curr_dep->randoms[i] & bit_i2_rands[i]) {
      has_i2_rand = 1;
    }
  }
  ...
  if (has_i1_rand) {
    ...
    memcpy(deps1[*deps1_length]->randoms, curr_dep->randoms,
           RANDOMS_MAX_LEN * sizeof(*curr_dep->randoms));
    (*deps1_length)++;
  } else if (has_i2_rand) {
    deps2[*deps2_length]->secrets[0] = curr_dep->secrets[0];
    deps2[*deps2_length]->secrets[1] = curr_dep->secrets[1];
    memcpy(deps2[*deps2_length]->randoms, curr_dep->randoms,
           RANDOMS_MAX_LEN * sizeof(*curr_dep->randoms));
    (*deps2_length)++;
  }
  ...
  for (int i = 0; i < bit_mult_len; i++) {
    ...
    while (mult_elem != 0) {
      ...
      BitDepVector* left  = mult->bits_left;
      BitDepVector* right = mult->bits_right;
      for (int k = 0; k < left->length; k++) {
        update_mult_deps(c, bit_deps, deps1, deps2,
                         deps1_length, deps2_length, left->content[k],
                         bit_i1_rands, bit_i2_rands);
      }
      for (int k = 0; k < right->length; k++) {
        update_mult_deps(c, bit_deps, deps1, deps2,
                         deps1_length, deps2_length, right->content[k],
                         bit_i1_rands, bit_i2_rands);
      }
    }
  }
  ...
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < share_count; j++) {
      if (left[i] & (1ULL << j)) {
        for (int k = 0; k < secret_count; k++) {
          if (right[k]) {
            factorized_deps[i*share_count+j]->secrets[k] ^= right[k];
          }
        }
        for (int k = secret_count; k < non_mult_deps_count; k++) {
          if (right[k]) {
        ...
        for (int k = 0; k < secret_count; k++) {
          if (left[k]) {
            factorized_deps[i*share_count+j]->secrets[k] ^= left[k];
          }
        }
        for (int k = secret_count; k < non_mult_deps_count; k++) {
          if (left[k]) {
  ...
  for (int i = c->secret_count; i < non_mult_deps_count; i++) {
    if (left[i]) {
      for (int j = 0; j < secret_count; j++) {
        if (right[j]) {
          factorized_deps[i+rand_offset]->secrets[j] ^= right[j];
        }
      }
      ...
      for (int j = 0; j < secret_count; j++) {
        if (left[j]) {
          factorized_deps[i+rand_offset]->secrets[j] ^= left[j];
        }
      }
      for (int j = secret_count; j < non_mult_deps_count; j++) {
        if (left[j]) {
...
void factorize_mults(const Circuit* c, BitDep** local_deps,
  ...
  for (int i = 0; i < local_deps_len; i++) {
    ...
    if (has_i1_rand) {
      deps1[*deps1_length]->secrets[0] = dep->secrets[0];
      deps1[*deps1_length]->secrets[1] = dep->secrets[1];
      memcpy(deps1[*deps1_length]->randoms, dep->randoms,
             bit_rand_len * sizeof(*dep->randoms));
      // No need to copy multiplications
      (*deps1_length)++;
    } else if (has_i2_rand) {
      deps2[*deps2_length]->secrets[0] = dep->secrets[0];
      deps2[*deps2_length]->secrets[1] = dep->secrets[1];
      memcpy(deps2[*deps2_length]->randoms, dep->randoms,
             bit_rand_len * sizeof(*dep->randoms));
      // No need to copy multiplications
      (*deps2_length)++;
    ...
    if (has_i1_rand || has_i2_rand) continue;
    ...
    // After having the basic factorization, we need to extract the
    // new elements of |factorized_deps| into |dep1| or |deps2|
    // (depending on whether the contain randoms/inputs from the 1st
    // or 2nd input). This is fairly straightforward:
    //  for each element e of |factorized_deps|
    //      if e is not empty:
    //          if the index of e corresponds to an input/rand of the 1st input:
    //              add e to deps2
    //          else (the index of e corresponds to an input/rand of the 2nd input):
    //              add e to deps1
    int rand_offset = inputs_real_count - c->secret_count;
    for (int i = 0; i < factorized_deps_length; i++) {
      int rand_set = 0;
      ...
      int rand_idx = i < inputs_real_count ? i / c->share_count : i - rand_offset;
      if (i1_rands[rand_idx] && deps2 != NULL) {
        deps2[*deps2_length]->secrets[0] = factorized_deps[i]->secrets[0];
        deps2[*deps2_length]->secrets[1] = factorized_deps[i]->secrets[1];
        memcpy(deps2[*deps2_length]->randoms, factorized_deps[i]->randoms,
               bit_rand_len * sizeof(*factorized_deps[i]->randoms));
        (*deps2_length)++;
      } else if (i2_rands[rand_idx] && deps1 != NULL) {
        deps1[*deps1_length]->secrets[0] = factorized_deps[i]->secrets[0];
        deps1[*deps1_length]->secrets[1] = factorized_deps[i]->secrets[1];
        memcpy(deps1[*deps1_length]->randoms, factorized_deps[i]->randoms,
               bit_rand_len * sizeof(*factorized_deps[i]->randoms));
        (*deps1_length)++;
      } else {
        assert(deps1 == NULL || deps2 == NULL);
      }
    }
  }
...
int _verify_tuples(const Circuit* circuit, // The circuit
  ...
  for (int i = 0; i < deps12_max_length; i++) {
    deps1[i] = alloca(sizeof(**deps1));
    deps2[i] = alloca(sizeof(**deps2));
  }
  ...
  for (int i = 0; i < deps12_max_length; i++) {
    deps1_fact[i] = alloca(sizeof(**deps1_fact));
    deps2_fact[i] = alloca(sizeof(**deps2_fact));
  }
  int deps1_length_fact = 0, deps2_length_fact = 0;
  GaussRand deps1_rands_fact[deps12_max_length];
  GaussRand deps2_rands_fact[deps12_max_length];
  ...
  int first_invalid_mult_index = 0;
  int local_deps_to_mult_1_map[local_deps_max_size];
  int local_deps_to_mult_2_map[local_deps_max_size];
  local_deps_to_mult_1_map[0] = local_deps_to_mult_2_map[0] = 0;

  int first_invalid_mult_index_fact = 0;
  int local_deps_to_mult_1_map_fact[local_deps_max_size];
  int local_deps_to_mult_2_map_fact[local_deps_max_size];
  local_deps_to_mult_1_map_fact[0] = local_deps_to_mult_2_map_fact[0] = 0;
  ...
  do {
    ...
    first_invalid_mult_index = min(first_invalid_mult_index,
                                   first_invalid_local_deps_index);
    ...
    if (! contains_mults) {
      ...
      int first_invalid_mult_index_in_local_deps =
        tuple_to_local_deps_map[first_invalid_mult_index];
      first_invalid_mult_index = comb_len;

      deps1_length = local_deps_to_mult_1_map[first_invalid_mult_index_in_local_deps];
      deps2_length = local_deps_to_mult_2_map[first_invalid_mult_index_in_local_deps];
      int up_to_date_deps1_length = deps1_length;
      int up_to_date_deps2_length = deps2_length;

      for (int i = first_invalid_mult_index_in_local_deps; i < local_deps_len; i++) {
        local_deps_to_mult_1_map[i] = deps1_length;
        local_deps_to_mult_2_map[i] = deps2_length;

        // Checking if the dep |idx| is random (ie, it contains a
        // random from |circuit->out_rands|)
        int contains_out_rand = 0;
        for (int j = 0; j < bit_rand_len; j++) {
          if (local_deps[i]->randoms[j] & bit_out_rands[j]) {
            contains_out_rand = 1;
            break;
          }
        }
        if (contains_out_rand) continue;

        update_mult_deps(circuit, bit_deps, deps1, deps2,
                         &deps1_length, &deps2_length, local_deps[i],
                         bit_i1_rands, bit_i2_rands);
      }

      // 3- Apply Gauss on both tuples
      for (int i = up_to_date_deps1_length; i < deps1_length; i++) {
        gauss_step(deps1[i], deps1, deps1_rands, bit_rand_len, bit_mult_len, i);
        set_gauss_rand(deps1, deps1_rands, i, bit_rand_len);
      }

      for (int i = up_to_date_deps2_length; i < deps2_length; i++) {
        gauss_step(deps2[i], deps2, deps2_rands, bit_rand_len, bit_mult_len, i);
        set_gauss_rand(deps2, deps2_rands, i, bit_rand_len);
      }


      int is_failure = set_contained_shares(leaky_inputs, secret_deps, deps1, deps1_rands,
                                            deps1_length, secret_count, t_in, comb_free_space,
                                            shares_to_ignore, PINI)
        // Using "+" rather than "||" to avoid short-circuit
        + set_contained_shares(leaky_inputs, secret_deps, deps2, deps2_rands,
                               deps2_length, secret_count, t_in, comb_free_space,
                               shares_to_ignore, PINI);
      ...
      deps1_length_fact =
        local_deps_to_mult_1_map_fact[first_invalid_mult_index_in_local_deps_fact];
      deps2_length_fact =
        local_deps_to_mult_2_map_fact[first_invalid_mult_index_in_local_deps_fact];
      int up_to_date_deps1_length_fact = deps1_length_fact;
      int up_to_date_deps2_length_fact = deps2_length_fact;

      for (int i = first_invalid_mult_index_in_local_deps_fact; i < local_deps_len; i++) {
        local_deps_to_mult_1_map_fact[i] = deps1_length_fact;
        local_deps_to_mult_2_map_fact[i] = deps2_length_fact;

        // TODO: it's more efficient to call factorize_mults only once...
        factorize_mults(circuit, &local_deps[i], deps1_fact, deps2_fact,
                        &deps1_length_fact, &deps2_length_fact, 1);
      }

      // Apply Gauss on both tuples
      for (int i = up_to_date_deps1_length_fact; i < deps1_length_fact; i++) {
        gauss_step(deps1_fact[i], deps1_fact, deps1_rands_fact, bit_rand_len, bit_mult_len, i);
        set_gauss_rand(deps1_fact, deps1_rands_fact, i, bit_rand_len);
      }
      for (int i = up_to_date_deps2_length_fact; i < deps2_length_fact; i++) {
        gauss_step(deps2_fact[i], deps2_fact, deps2_rands_fact, bit_rand_len, bit_mult_len, i);
        set_gauss_rand(deps2_fact, deps2_rands_fact, i, bit_rand_len);
      }

      secret_deps[0] = secret_deps[1] = 0;
      leaky_inputs[0] = leaky_inputs[1] = 0;

      is_failure = set_contained_shares(leaky_inputs, secret_deps,
                                        deps1_fact, deps1_rands_fact,
                                        deps1_length_fact, secret_count, t_in,
                                        comb_free_space, shares_to_ignore, PINI)
        // Using "+" rather than "||" to avoid short-circuit
        + set_contained_shares(leaky_inputs, secret_deps,
                               deps2_fact, deps2_rands_fact,
                               deps2_length_fact, secret_count, t_in,
                               comb_free_space, shares_to_ignore, PINI);
